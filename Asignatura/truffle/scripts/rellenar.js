module.exports = async callback => {
    ...
    try {
        ...
        // AÃ±adir profesores:
        console.log("Añadir profesores:");
        console.log("Cuenta del primer profesor =", profe1Addr);
        console.log("Cuenta del segundo profesor =", profe2Addr);
    
        await asignatura.addProfesor(profe1Addr, "Santiago Pavón");
        await asignatura.addProfesor(profe2Addr, "Joaquí­n Salvachúa");
        ...
        await asignatura.automatricula("Eva Martí­nez", "11111111A", "em@dominio.es", {from: evaAccount});
        await asignatura.automatricula("Jose Redondo", "22222222B", "jr@sitio.com", {from: pepeAccount});
        await asignatura.automatricula("Luis Martí­n",  "33333333C", "lm@aqui.com", {from: luisAccount});
    
        console.log("Crear evaluaciones:");
        await asignatura.creaEvaluacion("Primera Práctica",  Date.now() + 60 * 24 * 3600000, 20, 300, {from: coordinadorAccount});
        await asignatura.creaEvaluacion("Segundo Práctica", Date.now() + 120 * 24 * 3600000, 30, 400, {from: coordinadorAccount});
